...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!(file instanceof File)) {
      return NextResponse.json({ error: "请上传截图或视频文件" }, { status: 400 });
    }
    if (!MIME_TO_EXT[file.type]) {
      return NextResponse.json(
        { error: "只支持 PNG、JPG、JPEG、WebP、MP4、WebM、MOV" },
        { status: 400 }
      );
    }
    const isVideo = file.type.startsWith("video/");
    const maxBytes = isVideo ? MAX_SOURCE_VIDEO_BYTES : MAX_SOURCE_IMAGE_BYTES;
    ...
    const fileType: ProductSourceMediaAsset["fileType"] = file.type.startsWith(
      "video/"
    )
      ? "video"
      : "image";
    ...
    const asset: ProductSourceMediaAsset = {
      ...
      caption: guessCaption(file.name, fileType),
...
function guessCaption(
  fileName: string,
  fileType: ProductSourceMediaAsset["fileType"]
): string {
  const base = stripExtension(fileName);
  if (/(agent|chat|bot|智能体|对话|问答)/i.test(fileName)) {
    return fileType === "video" ? "智能体操作视频" : "智能体页面截图";
  }
  if (/(demo|演示|操作|流程)/i.test(fileName)) {
    return fileType === "video" ? "产品演示视频" : "产品流程截图";
  }
  return base || (fileType === "video" ? "产品演示视频" : "产品页面截图");
}
