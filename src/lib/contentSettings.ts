...
export function getContentLengthInstruction(id?: ContentLength): string {
  ...
  if (length === "deep") {
    return [
      "篇幅要求:深度文章,约 1800-2400 字。",
      "结构要求:开头钩子 + 4-5 个完整章节 + 克制结尾,适合竞品对比、行业观点或体系拆解。",
      "表达要求:信息密度更高,但每个技术点仍必须落到角色、动作和工作变化;不得编造数据。",
    ].join("\n");
  }
  return [
    "篇幅要求:标准文章,约 1200-1600 字。",
    "结构要求:开头钩子 + 3-4 个章节 + 自然行动建议,适合正式公众号产品稿。",
